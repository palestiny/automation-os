...
def test_candidate_rejects_empty_name():
    with pytest.raises(ValueError, match="name"):
        WorkflowCandidate.create("", ["create_short_video"], steps=[WorkflowCandidateStep.create("Step", "content.acquire")])


def test_candidate_rejects_empty_goal():
    with pytest.raises(ValueError, match="non-empty"):
        WorkflowCandidate.create("Workflow", [""], steps=[WorkflowCandidateStep.create("Step", "content.acquire")])
...
def test_candidate_rejects_duplicate_goals():
    with pytest.raises(ValueError, match="unique"):
        WorkflowCandidate.create(
            ...
            steps=[WorkflowCandidateStep.create("Step", "content.acquire")],
        )
