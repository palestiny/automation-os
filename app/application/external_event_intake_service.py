...
class ExternalEventIntake:
    """Normalize external events and delegate only through trigger invocation."""
    ...
    def intake(self, external_event: ExternalEvent) -> tuple[Execution, ...]:
        ...
        key = self._idempotency_key(external_event)
        return self._trigger_invocation.invoke(event, idempotency_key=key)
